...
\/____________/\_______\/    \/_________/   \_____\/
      ...
      list_dir(folder);
        ...
        cout << "\033[31m"
             << "(IMPORTANT NOTE) Imaginary answers for Quadratic equations are "
                ...
                "imaginary part."
             << "\033[37m" << '\n';
        cout << "Example: (2,3) would be 2+3i. (4,0) would be 4. (0,5) would be 5i"
             << '\n';
        cout << '\n';
        cout << "\033[32m"
        ...
        if (type == "linear") {
            ...
            cout << '\n'
                 << "\033[34m"
                 << "Step-by-Step Solution for Linear Equation:\n"
                 << "1. Given equation: y = " << m << "x + " << b << '\n'
                 << "2. Set y = 0 for solving x: 0 = " << m << "x + " << b << '\n'
                 << "3. Rearrange to solve for x: x = -b / m\n"
                 << "4. Substitute values: x = -" << b << " / " << m << '\n'
                 << "5. Simplify: x = " << answer << '\n'
                 << "\033[37m" << '\n';
            
        } else if (type == "quadratic") {
            complex<double> a(1, 0), b(0, 0), c(0, 0);
            ...
            complex<double> discr = sqrt((b * b) - (complex<double>(4, 0) * a * c));
            complex<double> denom = 2.0 * a;

            complex<double> ans1 = (-b + discr) / denom;
            complex<double> ans2 = (-b - discr) / denom;

            cout << '\n'
                 << "\033[34m"
                 << "Step-by-Step Solution for Quadratic Equation:\n"
                 << "1. Given equation: y = " << a << "x^2 + " << b << "x + " << c << '\n'
                 << "2. Calculate the discriminant: D = b^2 - 4ac = " << (b * b) - (complex<double>(4, 0) * a * c) << '\n'
                 << "3. Compute square root of discriminant: sqrt(D) = " << discr << '\n'
                 << "4. Calculate roots using the quadratic formula:\n"
                 << "   x1 = (-b + sqrt(D)) / (2a)\n"
                 << "   x2 = (-b - sqrt(D)) / (2a)\n"
                 << "5. Substitute values:\n"
                 << "   x1 = (" << -b << " + " << discr << ") / " << denom << " = " << ans1 << '\n'
                 << "   x2 = (" << -b << " - " << discr << ") / " << denom << " = " << ans2 << '\n'
                 << "\033[37m" << '\n';
            
        } else {
            cout << "\033[31m"
                 << "Invalid equation type. Please enter 'linear' or 'quadratic'."
                 << "\033[37m" << '\n';
        }
    } // end of SOLVE
      ...
      if (r == 0) {
        ...
        // Input the expression as a string
        string exprStr;
        cout << "Enter a mathematical expression in terms of x (e.g., 3*x**2 + 2*x - 5): ";
        getline(cin, exprStr);

        // Parse the expression
        RCP<const Basic> expr = parse(exprStr);

        // Compute the derivative
        RCP<const Basic> derivative = diff(expr, x);
